        ...
        <div className="flex flex-col gap-4">
          ...
          {content.techSpecs.map((row, i) => (
            <div key={i} className="flex gap-3 items-start">
              ...
              <button type="button" onClick={() => removeTechSpec(i)} className="font-heading text-[10px] uppercase tracking-widest border border-red-900/30 text-red-500/40 px-3 py-2.5 mt-5 hover:border-red-500/60 hover:text-red-400 transition-all flex-shrink-0">
                Remove
              </button>
            </div>
          ))}
          <button type="button" onClick={addTechSpec} className="self-start font-heading text-xs uppercase tracking-widest border border-white/12 text-white/40 px-4 py-2 hover:border-white/25 hover:text-white transition-all flex items-center gap-2">

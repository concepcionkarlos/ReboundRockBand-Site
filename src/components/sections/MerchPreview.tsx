...
const categoryIcons: Record<string, React.ReactNode> = {
  tshirt: (
    <svg className="w-10 h-10 text-brand-muted/25" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1} aria-hidden="true">
          ...
          {visible.map((item) => (
            ...
            >
              ...
              <div className="flex flex-col gap-1 pt-4">
                <h3 className="font-display text-lg text-white uppercase leading-tight">{item.name}</h3>
                <div className="font-display text-xl text-brand-red leading-none">${item.price}</div>
              </div>
            </Link>
          ))}
        </div>
